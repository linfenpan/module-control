 ...
 * @desc 脚本间依赖关系的排列，应该用“托普排序”，但现在暂时没性能问题，就放过它吧~
 */
...
var proto = {
	...
	_compileFirst: function(declare){
		...
		var pkg = this._pkg[module] || (this._pkg[module] = this._modules.readPkg(module), this._pkg[module]);
		if(version){
			pkg.path = version;
		}
		return item;
	...
	_compileConfig: function(item){
		...
		if(this._modules.exist(folder)){
			...
			// 包含表层文件..
			if(mod["include"]){
				res.push.apply(res, this._modules.find("!("+ item.root +".json)", folder, false));
			}
			
			// 检查version版本..
			var versionPath = path.join(folder, version);
			
			if(mod["all"] && this._modules.exist(versionPath)){
				// if(folder != item.module || versionPath != folder){
					res.push.apply(res, this._modules.find("*", versionPath, mod.all));
				// }
			}else{
...
var mcompiler = module.exports = proto;
